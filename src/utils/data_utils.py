...
def make_aggregate(vocab_size, dataset_size, min_length=4, max_length=16, seed=0):
    ...
    for _ in range(dataset_size):
        ...
        aggregate_value = round(sum(sent) / len(sent), 1)  # Calculate the mean and round to one decimal place
        aggregated_sent = [aggregate_value] * l  # Create the aggregate sentence
        
        sents.append([BOS] + list(map(str, sent)) + [EOS])  # Add BOS and EOS tokens
        tags.append([PAD] + list(map(str, aggregated_sent)) + [PAD])  # Add PAD tokens
